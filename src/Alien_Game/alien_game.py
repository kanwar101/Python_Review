	...
	def __init__ (self, width, height):
		...
		while not done:
			if len(self.aliens)==0:
				self.displayText("Victory - Game Over")
			...
			for alien in self.aliens:
				...
				if(alien.y>height):
					self.lost=True
					self.displayText("DIED - GAME OVER!!!")
...
class Hero:
	...
	def draw (self):
		...
		pygame.draw.rect(self.game.screen, (200,250,250), pygame.Rect(self.x, self.y, 8,5))
...
class Rocket:
	...
	def draw(self):
		pygame.draw.rect(self.game.screen, (242, 52, 110), pygame.Rect(self.x, self.y, 2,4))
		self.y -=2
